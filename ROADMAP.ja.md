...
`0.3.x` (pre-release)。コア機能は一通り揃い、`templates/wasm/` の手順で WASM ビルド可能。
今後は API 安定化と未着手領域（macOS ネイティブ統合・物理単位レイアウト）の実装が中心。

リリースラインは **`0.3.x` の一本**。`0.2.x` 保守ブランチは `v0.3.1` で `main` に合流済みで、
