...
**ステータス**: pre-release (`0.20.0`)。コア機能は実装済み、`templates/wasm/` の手順で WASM ビルド可能。利用側は **git タグ依存**が前提です（クレート間が path 依存で、シェーダーをクレート外から `include_str!` しているため）。各版の内容は [CHANGELOG.md](CHANGELOG.md)、この先の予定は [ROADMAP.ja.md](ROADMAP.ja.md) を参照。
...
ツリーは OS まで届きます。native ではアプリの窓に [accesskit](https://accesskit.dev) のアダプタが付くので、VoiceOver / NVDA / Orca から読み上げられ、**操作もできます** — スクリーンリーダーがボタンを押すと、マウスで押したときと同じ処理が走ります。支援技術が動いていないあいだは何も送らないので、付いていることの費用はほぼゼロです。

読み上げの順は**書いた順**で、ただの文字もツリーに入ります（押せるものだけでなく本文も読まれます）。名前は `.label()` があればそれ、無ければ**中の文字**なので、`button("保存")` はそのままで名乗ります。出るのは**いま画面に見えているものだけ**です — 一覧のスクロールで外に出た行は、戻ってくるまで出てきません。
...
```toml
[dependencies]
sabitori = { git = "https://github.com/Mutafika/sabitori", tag = "v0.20.0", features = ["hot-reload"] }
```
