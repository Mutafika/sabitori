...
4. **`THIRD-PARTY-LICENSES.html` を再生成する。**
   ...
   > **なぜリリース時なのか。** CI は「依存が漏れなく載っているか」を
   > **クレート名だけ**で見ている。`Cargo.lock` は追跡していないので、CI は毎回
   > まっさらに解決して最新の patch を拾う。バージョンまで CI で比べると、
   > リポジトリに何の変更が無くても上流が publish しただけで赤くなってしまう。
   >
   > 一方、**配布物に載るバージョンは正確であってほしい**。そこを揃えられるのが
