...
//! 動かしている限り永久に分からない形。
//!
//! ここで固定しているのは **example は自給自足であること**。 システムフォント
//! (native) にも組み込みフォント (wasm) にも寄りかからず、 自分が
//! `include_bytes!` した物だけで自分の描く字を全部描けること。
//!
//! 組み込みフォントに寄りかかると、 **native と wasm で別のフォントで描かれる**
//! ことになる — 組み込みは wasm にしか積まれないので、 native では
//! システムフォントが埋めてしまう。 同じソースが環境ごとに違う見た目になる
//! のは、 それ自体が次のバグの温床。
//!
//! ## 何を見ていないか
//!
