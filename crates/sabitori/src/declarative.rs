...
pub trait DeclarativeApp: 'static {
    ...
    /// ## wasm では
    ///
    /// ブラウザにシステムフォントは無い。 `builtin-font-jp` feature (既定で
    /// 有効) が wasm ビルドに HackGen (白源) を積むので、 **何も返さなくても
    ...
    ///
    /// **native と wasm で見た目を揃えたいなら、 組み込みに寄りかからず必ず
    /// ここで渡すこと。** 組み込みは wasm にしか積まれないので、 native では
    /// システムフォントが埋めてしまい、 同じソースが環境ごとに違う顔になる。
    ///
    /// ```ignore
