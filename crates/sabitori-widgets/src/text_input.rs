...
pub fn text_area(
    ...
    div()
        ...
        // 中身が本来の高さを主張する。
        .flex_col()
        // **箱は指定した行数ぶん。** 枠・背景・フォーカス枠を持っているのは
        // 中身 (`field`) の側なので、中身が中身なりの高さしか持たないと、
        // 空欄では 1 行ぶんの枠しか描かれず、下に指定した行数ぶんの透明な
        // 領域が残る — 押しても欄に入らない帯ができる (#70)。
        // 最低でもビューポートぶんの高さを持たせて、`<textarea rows=4>` と
        // 同じ「行数ぶんの箱に枠が付き、中身がその中でスクロールする」形に
        // する。溢れたら下の `shrink(0.0)` のとおり中身が伸びてスクロールする。
        .child(inner.min_h(Px(viewport_h)).shrink(0.0))
}
